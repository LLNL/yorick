...
@comment $Id: yorick.tex,v 1.1.1.1 2005-09-18 22:05:08 dhmunro Exp $
@comment -----------------------------------------------------------------
...
even for non-numeric objects, like the data type string.@refill
...
any effect outside @kbd{q_out}.@refill
...
function (@pxref{Customizing}).@refill
...
See @file{Y_SITE/i-start} for examples of how to do this.@refill
...
(red, green, blue), with 0 minimum intensity and 255 maximum.@refill
...
Yorick will silently overwrite the existing file.@refill
...
you will need to use a modified @kbd{Y_HOME/Makepkg} file to make it
work, but it shouldn't present insurmountable difficulties.@refill
...
compiled package available to the interpreter.@refill
...
which distinguishes it from an interpreted package:@refill
...
directory to hold the source.)@refill
...
compiled code and yorick's internal compiled code.@refill
...
long) list of inputs an produce the corresponding list of outputs.@refill
...
unable to determine by its examination of the source files.@refill
...
you can possibly avoid it.@refill
...
command lines; read @file{Y_HOME/Makepkg} to learn about others:@refill
...
@kbd{my_func} to a compiled function @kbd{Y_my_func} of type BuiltIn,
which you are responsible for writing.  See @file{Y_HOME/ydata.h} for
...
declared in @file{Y_HOME/ydata.h}, and push its result back onto the
stack.  The distribution source code in @file{yorick/ystr.c} is the
most up to date example of how to write BuiltIn functions directly.@refill
